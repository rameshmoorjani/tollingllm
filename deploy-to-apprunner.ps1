...
if ($serviceExists) {
    Write-Host "ℹ️  Service exists, starting new deployment..." -ForegroundColor Cyan
...
"@
        $trustPolicy | Out-File -FilePath /tmp/trust-policy.json
    ...
    aws apprunner create-service `
        ...
        --source-configuration ImageRepository="{ImageIdentifier=$ECR_URI`:latest,ImageRepositoryType=ECR,ImageConfiguration={Port=5000}}" `
        --instance-configuration InstanceRoleArn=$roleArn `
        --region $AWS_REGION
...
for ($i = 1; $i -le 30; $i++) {
    ...
    if ($status -eq "RUNNING") {
        $appUrl = aws apprunner describe-service `
            --service-arn $serviceArn `
            --region $AWS_REGION `
